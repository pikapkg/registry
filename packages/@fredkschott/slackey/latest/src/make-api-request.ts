...
// TODO: Replace this stub with fetch, available soon on Pika
...
export function makeAPIRequest(requestOptions, callback) {
  ...
  requestOptions.json =
    typeof requestOptions.json !== 'undefined' ? requestOptions.json : true;
  ...
  request(requestOptions, function(err, response) {
    ...
    if (!response.body.ok) {
      callback(new SlackError(response.body.error, { response: response }));
      return;
    }
