 ...
 * @return {void}
 */
export function SlackOAuthClient(options) {
  options = options || {};
  ...
  if (!this.clientID) {
    throw new Error(
      'A "clientID" option is required to authorize users with the Slack API.'
    );
  }
  if (!this.clientSecret) {
    throw new Error(
      'A "clientSecret" option is required to authorize users with the Slack API.'
    );
  }
}
