...
export function SlackAPIClient(options) {
  ...
  if (!this.token) {
    throw new Error(
      'A "token" option is required to make calls to the Slack API.'
    );
  }
}
...
SlackAPIClient.prototype.send = function(method, options, callback) {
  ...
  switch (method) {
    case 'chat.postMessage':
      ...
      requestOptions.body = encodeQueryString(options);
      break;
    ...
    case 'files.upload':
      ...
      if (options.content) {
        ...
        requestOptions.body = encodeQueryString({ content: options.content });
